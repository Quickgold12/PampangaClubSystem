// ─────────────────────────────────────────────────────────────────────────────
// Global Moderation Queue — Reports.
//
// Audience: club advisers and faculty coordinators.
//
// Same shape as the pending-announcements queue but for reports. Each row
// links to that club's Reports screen where the approve/reject buttons live.
// ─────────────────────────────────────────────────────────────────────────────
import { useAuth } from '@/context/AuthContext'
import { useTheme } from '@/hooks/use-theme'
import { listPendingForReviewer } from '@/services/report.service'
import { ReportFeedItem } from '@/types'
...
export default function PendingReportsScreen() {
  ...
  const [rows, setRows] = useState<ReportFeedItem[]>([])
  ...
  return (
    <>
      <Stack.Screen options={{ title: 'Pending Reports', headerShown: true }} />
      <ScrollView
      ...
      >
        ...
        {rows.length === 0 ? (
          <View style={styles.empty}>
            <Text style={styles.emptyText}>No reports waiting for review.</Text>
          </View>
        ) : (
          rows.map((r) => (
            <Pressable
              key={r.id}
              onPress={() => router.push(`/club/${r.organization.id}/reports` as never)}
            ...
            >
              <View style={styles.cardTop}>
                ...
                {/* Activity vs Financial badge — useful for the reviewer to
                    triage which queue this entry belongs to mentally. */}
              ...
              <Text style={styles.cardMeta}>
                Submitted by {r.submitter?.full_name ?? 'Unknown'} • {formatDate(r.submitted_at)}
              </Text>
            </Pressable>
          ))
        )}
